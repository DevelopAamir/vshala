...
class WLSM_Api {
	...
	public static function register_rest_routes() {
		...
		// Student - Quiz Categories.
		register_rest_route(
			self::NS, self::STUDENT_PREFIX . '/quiz-categories',
			array(
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => array( 'WLSM_Api', 'student_quiz_categories' ),
				'permission_callback' => function( $request ) {
					return is_user_logged_in();
				}
			)
		...
		// Student - Quizes.
		register_rest_route(
			self::NS, self::STUDENT_PREFIX . '/quizes/(?P<quiz_category_id>\d+)',
			array(
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => array( 'WLSM_Api', 'student_quizes' ),
				'permission_callback' => function( $request ) {
					return is_user_logged_in();
				}
			)
		...
		// Student - Subjects.
		register_rest_route(
			self::NS, self::STUDENT_PREFIX . '/subjects',
			array(
				'methods'             => WP_REST_Server::READABLE,
	...
	// Student - Study material.
		register_rest_route(
			self::NS, self::STUDENT_PREFIX . '/study-material/(?P<subject_id>\d+)/(?P<material_type>[\w]+)/(?P<chapter>[\w]+)',
			array(
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => array( 'WLSM_Api', 'student_studymaterials' ),
				'permission_callback' => function( $request ) {
					return is_user_logged_in();
				}
			)
		...
		register_rest_route(
			self::NS, self::STUDENT_PREFIX . '/submit-homework',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( 'WLSM_Api', 'student_submit_homework' ),
				'permission_callback' => function( $request ) {
					return is_user_logged_in();
				}
			)
	...
	public static function student_profile( $request ) {
		...
		try {
			...
			$other_details  = array(
				...
				'father_phone' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->father_phone ) ),
				'father_occupation' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->father_occupation ) ),
				'mother_name' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->mother_name ) ),
				'mother_phone' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->mother_phone ) ),
				'mother_occupation' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->mother_occupation ) ),
				'mobileno' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->phone ) ),
				'email' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->email ) ),
				'dob' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->dob ) ),
				'blood_group' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->blood_group ) ),
				'address' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->address ) ),
				'admission_date' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->admission_date ) ),
				'gender' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->gender ) ),
				'religion' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->religion ) ),
				'caste' => esc_html( WLSM_M_Staff_Class::get_name_text( $student_common_details->caste ) ),
   ...
   // Student - Quiz Categories.
	public static function student_quiz_categories( $request ) {
		$user_id = get_current_user_id();
		...
		try {
			...
			// Query.
			$quiz_categories_query = WLSM_M::quiz_categories_query($class_school_id);

			// Total.
			$quiz_categories_total = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(1) FROM ({$quiz_categories_query}) AS combined_table") );

			// Current page and per page.
			$quiz_categories_page     = isset( $params['quiz_categories_page'] ) ? absint( $params['quiz_categories_page'] ) : 1;
			$quiz_categories_per_page = isset( $params['quiz_categories_per_page'] ) ? absint( $params['quiz_categories_per_page'] ) : WLSM_M::quiz_categories_per_page();

			// Page offset.
			$quiz_categories_page_offset = ( $quiz_categories_page * $quiz_categories_per_page ) - $quiz_categories_per_page;

			// Paginated data.
			$quiz_categories = $wpdb->get_results( $wpdb->prepare( $quiz_categories_query . ' ORDER BY ID DESC LIMIT %d, %d', $quiz_categories_page_offset, $quiz_categories_per_page ) );

			// Paginated response.
			$quiz_categories_data       = array();
			$quiz_categories_pagination = array(
				'current_page' => $quiz_categories_page,
				'per_page'     => $quiz_categories_per_page,
			);

			// Format response.
			if ( count( $quiz_categories ) ) {
				$today = new DateTime();
				$today->setTime( 0, 0, 0 );
				...
				foreach ( $quiz_categories as $key => $quiz_category ) {
					...
					$quiz_categories_data[] = array(
						'id'       => $quiz_category->id,
						'title'    => esc_html( stripslashes( $quiz_category->title ) ),
						'description'  => esc_html( stripslashes( $quiz_category->description ) )

					);
				}

				$quiz_categories_pagination['total_pages']   = ceil( $quiz_categories_total / $quiz_categories_per_page );
				$quiz_categories_pagination['total_records'] = $quiz_categories_total;
			}

			$success = true;
			$message = esc_html__( 'Quiz Categories retrieved successfully.', 'school-management' );

			$response_data['quiz_categories'] = array(
				'new_notice_icon' => esc_url( WLSM_PLUGIN_URL . 'assets/images/newicon.gif' ),
				'data'            => $quiz_categories_data,
				'pagination'      => $quiz_categories_pagination
			);
	...
	// Student - Fee Invoice.
	public static function student_quizes( $request ) {
		$user_id = get_current_user_id();
			try {
			global $wpdb;
			...
			$quiz_category_id = isset( $params['quiz_category_id'] ) ? absint( $params['quiz_category_id'] ) : 0;

			            // Query.
			$quizes_query = WLSM_M::quizes_query();

			// Total.
			$quizes_total = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(1) FROM ({$quizes_query}) AS combined_table WHERE quiz_category_id=%d",$quiz_category_id) );

			// Current page and per page.
			$quizes_page     = isset( $params['quizes_page'] ) ? absint( $params['quizes_page'] ) : 1;
			$quizes_per_page = isset( $params['quizes_per_page'] ) ? absint( $params['quizes_per_page'] ) : WLSM_M::quizes_per_page();

			// Page offset.
			$quizes_page_offset = ( $quizes_page * $quizes_per_page ) - $quizes_per_page;

			// Paginated data.
			$quizes = $wpdb->get_results( $wpdb->prepare( $quizes_query . ' WHERE quiz_category_id=%d ORDER BY ID DESC LIMIT %d, %d', $quiz_category_id,$quizes_page_offset, $quizes_per_page ) );

			// Paginated response.
			$quizes_data       = array();
			$quizes_pagination = array(
				'current_page' => $quizes_page,
				'per_page'     => $quizes_per_page,
			);
			...
			if ( count( $quizes ) ) {
				...
				foreach ( $quizes as $key => $quiz ) {
					...
					$quizes_data[] = array(
						'id'       => $quiz->id,
						'title'    => esc_html( stripslashes( $quiz->title ) ),
						'description'  => esc_html( stripslashes( $quiz->description ) )

					);
				}

				$quizes_pagination['total_pages']   = ceil( $quizes_total / $quizes_per_page );
				$quizes_pagination['total_records'] = $quizes_total;
			}

			$success = true;
			$message = esc_html__( 'Quizes retrieved successfully.', 'school-management' );

			$response_data['quizes'] = array(
				'new_notice_icon' => esc_url( WLSM_PLUGIN_URL . 'assets/images/newicon.gif' ),
				'data'            => $quizes_data,
				'pagination'      => $quizes_pagination
			);
...
// Student - Subjects.
	public static function student_subjects( $request ) {
		$user_id = get_current_user_id();
		...
		try {
			...
			// Query.
			$subjects_query = WLSM_M::get_subjects_query($class_school_id);


			// Paginated data.
			$subjects = $wpdb->get_results( $wpdb->prepare( $subjects_query." where class_school_id=%d",$class_school_id) );
			...
			if ( count( $subjects ) ) {
				foreach ( $subjects as $key => $subject ) {
					$subjects_data[] = array(
						'id'          => $subject->ID,
						'subjectName'       => esc_html( WLSM_M_Staff_Class::get_name_text( $subject->label ) ),
						'code'       => esc_html( WLSM_M_Staff_Class::get_name_text( $subject->code ) ),
						'type'       => esc_html( WLSM_M_Staff_Class::get_name_text( $subject->type ) ),
						'teacher_name'       => ""
					);
				}
	...
	// Student - Study materials.
	public static function student_studymaterials( $request ) {
		$user_id = get_current_user_id();
		...
		try {
			...
			$subject_id = isset( $params['subject_id'] ) ? absint( $params['subject_id'] ) : 0;
	         $material_type = isset( $params['material_type'] ) ?$params['material_type'] : "";
	         $chapter = isset( $params['chapter'] ) ?$params['chapter'] : "";

			// Query.
			$study_materials_query = WLSM_M::get_study_materials_query();
            $study_materials_query.=" WHERE cssm.class_school_id=$class_school_id and cssm.study_material_subject_id=$subject_id and sm.material_type='$material_type' and chapter='$chapter'  GROUP BY sm.ID";
			// Total.
			$study_materials_total = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(1) FROM ({$study_materials_query}) AS combined_table" ) );
			...
			// Paginated data.
			$study_materials = $wpdb->get_results( $wpdb->prepare( $study_materials_query . ' ORDER BY sm.ID DESC LIMIT %d, %d', $study_materials_page_offset, $study_materials_per_page ) );
			...
			if ( count( $attachments ) ) {
				foreach ( $attachments as $attachment ) {
					...
					$study_materials_data[] = array(
						...
						'url' => esc_html( stripslashes( $study_material->url ) ),
...
$attachments = $homework->attachments;
	...
	public static function student_submit_leave_request( $request ) {
    	...
    	// Student - Submit homework.
	public static function student_submit_homework( $request ) {
		$user_id = get_current_user_id();
		...
		//	$homework_update_id = isset($params['homework_update']) ? sanitize_text_field($params['homework_update']): '';
		//	$homework_sub_id = isset($params['homework_sub_id']) ? sanitize_text_field($params['homework_sub_id']): '';
			//$attachment = ( isset( $_FILES['attachments'] ) && is_array( $_FILES['attachments'] ) ) ? $_FILES['attachments'] : NULL;

			if (empty($submission_id)) {
				throw new Exception(esc_html__('Please Enter Submission Subject', 'school-management'));
			}

			if (empty($description)) {
				throw new Exception(esc_html__('Please Enter discription', 'school-management'));
			}

			/*if (isset($attachment['tmp_name']) && !empty($attachment['tmp_name'])) {
				if (!WLSM_Helper::is_valid_file($attachment, 'attachment')) {
					throw new Exception(esc_html__('Please provide attachment PDF format.', 'school-management'));
				}
			}*/
